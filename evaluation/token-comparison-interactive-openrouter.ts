/**
 * Token Comparison Chat - Interactive MCP Flow (OpenRouter)
 *
 * Interactive chat interface based on ollama-chat-client with token tracking using OpenRouter.
...
class InteractiveMCPChatClient {
  ...
  async initialize(): Promise<void> {
    displayMessage('system', 'Checking OpenRouter connection...');
    ...
    const serverPath = path.join(__dirname, '..', 'examples', 'servers', 'stdio-server');
  ...
  async chat(): Promise<void> {
    ...
    while (running) {
      try {
        ...
        // Get response from OpenRouter (may include tool calls)
  ...
  /**
   * Process the conversation with OpenRouter
   */
  ...
  private displayHeader(): void {
    ...
    console.log(colorize('  Token Comparison Chat - Interactive MCP Flow (OpenRouter)', 'bright'));
    console.log(colorize('═'.repeat(60), 'cyan'));
    console.log();
